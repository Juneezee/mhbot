...
// // Labyrinth
var bestLabyBase = ["Prestige Base", "Minotaur Base", "Labyrinth Base"];
var objCodename = {
  FEALTY: "y",
  TECH: "h",
  SCHOLAR: "s",
...
var arrHallwayOrder = ["sp", "mp", "lp", "ss", "ms", "ls", "se", "me", "le"];
var objDefaultLaby = {
  districtFocus: "None",
  between0and14: ["lp"],
...
var scriptVersion = chrome.runtime.getManifest().version;
...
function FinalizePuzzleImageAnswer(answer) {
  ...
  if (answer.length != 5) {
    ...
    var puzzleAns = document.getElementsByClassName(
      "mousehuntPage-puzzle-form-code"
    )[0];
    ...
    var puzzleSubmit = document.getElementsByClassName(
      "mousehuntPage-puzzle-form-code-button"
    )[0];
...
function receiveMessage(event) {
  ...
  if (
    ...
    if (event.data.indexOf("~") > -1) {
      ...
      if (saveKRImage) {
        var processedImg = event.data.substring(
...
function CallKRSolver() {
  ...
  if (debugKR) {
    ...
    img = document.getElementsByClassName(
      "mousehuntPage-puzzle-form-captcha-image"
    )[0];
...
function CheckKRAnswerCorrectness() {
  var puzzleForm = document.getElementsByClassName(
  ...
  var strTemp = "";
  var codeError = document.getElementsByClassName(
    "mousehuntPage-puzzle-form-code-error"
  );
...
function resumeKRAfterSolved() {
  ...
  var resumeButton = document.getElementsByClassName(
    ...
    if (fbPlatform) {
      if (
        ...
        if (!checkIntroContainer() && retrieveDataFirst()) {
          ...
          document.title = `Fail to retrieve data from page. Reloading in ${timeFormat(
            errorReloadTime
          )}`;
          window.setTimeout(function () {
            reloadPage(false);
...
function bwRift() {
  ...
  var objUser = JSON.parse(
    getPageVariable("JSON.stringify(user.quests.QuestRiftBristleWoods)")
  );
  var nIndex = -1;
  var nLootRemaining = objUser.progress_remaining;
  var nTimeSand = parseInt(
    objUser.items.rift_hourglass_sand_stat_item.quantity
  );
  ...
  if (strChamberName == "ACOLYTE") {
    ...
    console.plog(
      ...
      strStatus,
      "Obelisk:",
      objUser.minigame.acolyte_chamber.obelisk_charge,
      "Acolyte Sand:",
  ...
  console.plog(
    "Status:",
    objUser.chamber_status,
    "Name:",
    objUser.chamber_name,
    "Shortname:",
    strChamberName,
    "Index:",
    nIndex,
  ...
  if (
    !(
      objUser.status_effects.un.indexOf("default") > -1 ||
      objUser.status_effects.un.indexOf("remove") > -1
    ) ||
    !(
      objUser.status_effects.fr.indexOf("default") > -1 ||
      objUser.status_effects.fr.indexOf("remove") > -1
    ) ||
    !(
      objUser.status_effects.st.indexOf("default") > -1 ||
      objUser.status_effects.st.indexOf("remove") > -1
    )
  )
    nIndexBuffCurse = 8;
  else {
    ...
    if (classPortalContainer.length > 0) {
      ...
      if (objBWRift.choosePortal) {
        if (
          ...
          var arrIndices = [];
          var nRSCPot = parseInt(
            objUser.items.runic_string_cheese_potion.quantity
          );
          ...
          if (nIndexTemp > -1) {
            ...
            if (
              nTotalRSC < nMinRSC ||
              nTimeSand < objBWRift.minTimeSand[nIndexBuffCurse]
            ) {
          ...
          if (arrAL.length > 0 && arrRL.length > 0 && nIndexCustom > -1) {
            var nASCPot = parseInt(
              objUser.items.ancient_string_cheese_potion.quantity
            );
  ...
  var classLootBooster = document.getElementsByClassName(
    "riftBristleWoodsHUD-portalEquipment lootBooster mousehuntTooltipParent"
  )[0];
  var bPocketwatchActive =
    classLootBooster.getAttribute("class").indexOf("selected") > -1;
  var classButton = classLootBooster.getElementsByClassName(
  ...
  if (objTemp.activate) {
    bForce =
      objBWRift.specialActivate.forceDeactivate[nIndex] &&
      nLootRemaining <=
        objBWRift.specialActivate.remainingLootDeactivate[nIndex];
    if (bForce === bPocketwatchActive) bToggle = true;
  } else {
    bForce =
      objBWRift.specialActivate.forceActivate[nIndex] &&
      nLootRemaining <= objBWRift.specialActivate.remainingLootActivate[nIndex];
    if (bForce !== bPocketwatchActive) bToggle = true;
  }
  console.plog(
    "QQ Activated:",
...
function fortRox() {
  ...
  var objUser = JSON.parse(
    getPageVariable("JSON.stringify(user.quests.QuestFortRox)")
  );
  var nIndex = -1;
  ...
  } else checkThenArm(null, "bait", objFRox.bait[nIndex]);

  var bTowerActive = !(objUser.tower_status.indexOf("inactive") > -1);
  var nMana = parseInt(
    document.getElementsByClassName("fortRoxHUD-mana quantity")[0].textContent
  );
  console.plog(
    "Tower Active:",
    bTowerActive,
  ...
  if (nMana > 0 && nIndex > 0) {
    var classButton = document.getElementsByClassName(
      "fortRoxHUD-spellTowerButton"
    )[0];
    if (bTowerActive) {
...
function Halloween2014() {
  ...
  if (currentLocation.indexOf("Haunted Terrortories") > -1) {
    var areaName = document.getElementsByClassName(
      "halloween2014Hud-areaDetails-name"
    )[0].innerHTML;
    var warning = document.getElementsByClassName(
    ...
    if (isWarning) {
      var trickContainer = document.getElementsByClassName(
      ...
      if (trickContainer.children[2].getAttribute("class") == "armNow active") {
        ...
        console.debug(
          "Currently armed: Treat cheese, Going to arm Trick cheese"
        );
...
function Halloween2016() {
  ...
  if (isWarning) {
    ...
    var i;
    var nSquareMin = 0;
    var classContent = document.getElementsByClassName(
    ...
    var stageContainer = document.getElementsByClassName(
  ...
  if (currentLocation.indexOf("Gnawnian Express") > -1) {
    ...
    if (onTrain == "false" || onTrain == 0) {
      if (
        ...
        charmArmed.indexOf("Greasy Glob") > -1 ||
        charmArmed.indexOf("Door Guard") > -1 ||
        charmArmed.indexOf("Dusty Coal") > -1 ||
        charmArmed.indexOf("Black Powder") > -1 ||
        charmArmed.indexOf("Magmatic Crystal") > -1
      )
        disarmTrap("trinket");

      // TODO: Need to fix this
      if (
        trapArmed.indexOf("Supply Grabber") > -1 ||
        trapArmed.indexOf("Bandit Deflector") > -1 ||
        trapArmed.indexOf("Engine Doubler") > -1
      )
        checkThenArm("best", "weapon", [
          "S.L.A.C. II",
          "The Law Draw",
          "S.L.A.C.",
        ]);
      ...
      var nPhaseSecLeft = parseInt(
        getPageVariable("user.quests.QuestTrainStation.phase_seconds_remaining")
      );
      console.plog(
        "Current Phase:",
        strCurrentPhase,
...
function ges() {
  if (GetCurrentLocation().indexOf("Gnawnian Express Station") < 0) return;
  ...
  if (strCurrentPhase.indexOf("Supply Depot") > -1) {
    if (
      ...
      if (nTurn <= 0) {
        ...
        strStage = "SD_BEFORE";
        if (
          objGES.SD_BEFORE.trinket.indexOf("Supply Schedule") > -1 &&
        ...
        } else checkThenArm(null, "trinket", objGES.SD_BEFORE.trinket);
      } else {
        strStage = "SD_AFTER";
        if (objGES.SD_AFTER.trinket.indexOf("Supply Schedule") > -1)
          disarmTrap("trinket");
        else checkThenArm(null, "trinket", objGES.SD_AFTER.trinket);
      }
    }

    if (objGES.bLoadCrate) {
      var nCrateQuantity = parseInt(
        document
      ...
      if (
        ...
        nCrateQuantity >= objGES.nMinCrate
      )
        fireEvent(document.getElementsByClassName("phaseButton")[0], "click");
    }
    ...
    if (
      ...
      if (objGES.RR.trinket == "AUTO") {
        ...
        for (i = 0; i < classTrainCarArea.length; i++) {
          if (classTrainCarArea[i].className.indexOf("attacked") > -1) {
            ...
            console.plog(
              "Raiders Attack:",
              capitalizeFirstLetter(strAttack),
              ",",
              arrCharm[i],
              "Charm Quantity:",
              nCharmQuantity
      ...
      } else checkThenArm(null, "trinket", objGES.RR.trinket);
    }

    if (objGES.bUseRepellent) {
      var nRepellentQuantity = parseInt(
        document
          .getElementsByClassName("mouseRepellent")[0]
          .getElementsByClassName("quantity")[0].textContent
      );
    ...
    if (
      ...
      if (objGES.DC.trinket == "AUTO")
        ...
        if (arrCharm.indexOf(objGES.DC.trinket) > -1) {
          ...
          nCharmQuantity = parseInt(
            classCharms[0].children[nIndex].getElementsByClassName(
              "quantity"
            )[0].textContent
          );
          ...
          if (
            Number.isInteger(nCharmQuantity) &&
            nCharmQuantity > 0 &&
            charmArmed.indexOf(objGES.DC.trinket) < 0
          )
            fireEvent(classCharms[0].children[nIndex], "click");
        } else checkThenArm(null, "trinket", objGES.DC.trinket);
      }
    }

    if (objGES.bStokeEngine) {
      // get fuel nugget quantity
      var nFuelQuantity = parseInt(
        document
      ...
      if (
        ...
        nFuelQuantity >= objGES.nMinFuelNugget
      )
        fireEvent(document.getElementsByClassName("phaseButton")[0], "click");
    }
  } else {
    ...
    if (objGES.WAITING.trinket.indexOf(arrCharm) > -1) disarmTrap("trinket");
    else checkThenArm(null, "trinket", objGES.WAITING.trinket);
  }
...
function wwrift() {
  if (GetCurrentLocation().indexOf("Whisker Woods Rift") < 0) return;
  ...
  if (
    isNullOrUndefined(objWWRift.factionFocusNext) ||
    objWWRift.factionFocus === ""
  )
  ...
  if (objWWRift.factionFocus == "MBW_40_44") {
    ...
    if (nBar25 >= 3) {
      for (i = 0; i < objWWRift.rage.length; i++) {
        if (objWWRift.rage[i] >= objWWRift.MBW.minRageLLC) nBarMinRage++;
      }
    }
    ...
    if (objWWRift.MBW.rage4044.trinket[nIndex].indexOf("FSC") > -1) {
      ...
      if (nIndexCharm > -1) {
        if (objWWRift.rage[nIndexCharm] >= nLimit) {
          ...
          if (temp > -1)
            objWWRift.MBW.rage4044.trinket[nIndex] =
              objWWRift.funnelCharm[temp];
        } else objWWRift.MBW.rage4044.trinket[nIndex] = charmArmed;
      } else {
        temp = minIndex(objWWRift.rage);
        if (temp > -1)
    ...
    if (objWWRift.MBW.rage4548.trinket[nIndex].indexOf("FSC") > -1) {
      ...
      if (nIndexCharm > -1) {
        if (objWWRift.rage[nIndexCharm] >= nLimit) {
          temp = minIndex(objWWRift.rage);
          if (temp > -1)
            objWWRift.MBW.rage4548.trinket[nIndex] =
              objWWRift.funnelCharm[temp];
        } else objWWRift.MBW.rage4548.trinket[nIndex] = charmArmed;
      } else {
        temp = minIndex(objWWRift.rage);
        if (temp > -1)
    ...
    if (objWWRift.faction.trinket[nIndex].indexOf("FSC") > -1) {
      if (
        ...
        if (objWWRift.rage[temp] < nLastRage) {
          ...
          objWWRift.factionFocus = objWWRift.factionFocusNext;
          bResave = true;
        } else objWWRift.faction.trinket[nIndex] = objWWRift.funnelCharm[temp];
      }
    }
...
function BurroughRift(bCheckLoc, minMist, maxMist, nToggle) {
  ...
  var currentMistQuantity = parseInt(
...
function BRCustom() {
  if (GetCurrentLocation().indexOf("Burroughs Rift") < 0) return;
...
function livingGarden(obj) {
  ...
  var strStatus = "";
  if (Number.isNaN(estimateHunt)) strStatus = "Poured";
  else if (estimateHunt >= 35) strStatus = "Filled";
  ...
  if (strStatus == "Poured") {
    ...
    if (obj.LG.isAutoPour && !isNullOrUndefined(pourButton)) {
      ...
      if (document.getElementsByClassName("confirm button")[0]) {
        ...
        if (charmArmed.indexOf("Sponge") > -1) disarmTrap("trinket");
    ...
    if (!obj.LG.isAutoFill) {
      if (
        charmArmed.indexOf("Sponge") > -1 ||
        obj.LG.trinket.after.indexOf(charmArmed) > -1 ||
        charmArmed.indexOf(obj.LG.trinket.after) > -1
      )
        disarmTrap("trinket");
    } else {
      if (estimateHunt >= 28) checkThenArm(null, "trinket", "Sponge");
      else checkThenArm("best", "trinket", spongeCharm);
    }
    if (
      baitArmed.indexOf("Camembert") > -1 &&
      baitArmed.indexOf("Duskshade") < 0
    )
      checkThenArm(null, "bait", "Gouda");
  }
}
...
function lostCity(obj) {
  ...
  var isCursed =
    document.getElementsByClassName("stateBlessed hidden").length > 0;
...
function sandDunes() {
  var hasStampede = getPageVariable(
    "user.quests.QuestSandDunes.minigame.has_stampede"
  );
...
function twistedGarden(obj) {
  checkThenArm("best", "weapon", objBestTrap.weapon.hydro);
  var red = parseInt(
    document.getElementsByClassName("itemImage red")[0].innerText
  );
  var yellow = parseInt(
    document.getElementsByClassName("itemImage yellow")[0].innerText
  );
  var nEstimateHunt = -1;
  var charmArmed = getPageVariable("user.trinket_name");
  var strStatus = "";
  ...
  if (
    obj.TG.trinket.after.indexOf("Red") > -1 ||
  ...
  if (strStatus == "Poured") {
    ...
    if (!obj.TG.isAutoFill) {
      if (
        charmArmed.indexOf("Red") > -1 ||
        charmArmed.indexOf("Yellow") > -1 ||
        obj.TG.trinket.after.indexOf(charmArmed) > -1 ||
        charmArmed.indexOf(obj.TG.trinket.after) > -1
      )
        disarmTrap("trinket");
    } else {
      if (red <= 8 && yellow <= 8)
...
function cursedCity(obj) {
  ...
  var objCC = JSON.parse(
    getPageVariable("JSON.stringify(user.quests.QuestLostCity.minigame)")
  );
  var curses = "";
  var charmArmed = getPageVariable("user.trinket_name");
  console.plog(objCC);
  if (objCC.is_cursed === false) {
    ...
    if (
      obj.CC.trinket.after.indexOf("Bravery") > -1 ||
      obj.CC.trinket.after.indexOf("Shine") > -1 ||
      obj.CC.trinket.after.indexOf("Clarity") > -1
    )
...
function sandCrypts(obj) {
  ...
  checkThenArm(null, "bait", "Graveblossom");
  var salt = parseInt(
    document.getElementsByClassName("salt_charms")[0].innerText
  );
...
function ZTower() {
  ...
  if (location.indexOf("Seasonal Garden") > -1) {
    ...
    var ztTriesLeft = 5;
    retrieveMouseList();
    var intervalZT = setInterval(function () {
...
function seasonalGarden() {
  if (GetCurrentLocation().indexOf("Seasonal Garden") < 0) return;
  ...
  var nCurrentAmp = parseInt(
    getPageVariable("user.viewing_atts.zzt_amplifier")
  );
  var nMaxAmp = parseInt(
  ...
  if (nCurrentAmp + 1 >= nMaxAmp) {
    ...
    if (nCurrentAmp >= nMaxAmp && objSG.disarmBaitAfterCharged)
      disarmTrap("bait");
    else checkThenArm(null, "bait", objSG.bait[nSeason]);
  } else checkThenArm(null, "bait", objSG.bait[nSeason]);
}
...
function zugzwangTower() {
  ...
  var nProgressMystic = parseInt(
    getPageVariable("user.viewing_atts.zzt_mage_progress")
  );
  var nProgressTechnic = parseInt(
  ...
  if (objZT.weapon[nIndex] == "MPP/TPP") {
    ...
    if (objZT.focus.indexOf("MYSTIC") === 0)
      ...
      objZT.weapon[nIndex] =
        nIndex >= 7 ? "Blackstone Pass Trap" : "Obvious Ambush Trap";
  }
...
function balackCoveJOD() {
  ...
  if (bInJOD) nIndex = 0;
    ...
    for (i = 0; i < objBC.arrTide.length; i++) {
      objBC.arrAll = objBC.arrAll.concat(
        new Array(objBC.arrLength[i]).fill(objBC.arrTide[i])
      );
    }
    ...
    console.plog(
      "Current Tide:",
      objBC.arrAll[nIndexCurrentTide],
      "Index:",
      nIndex,
      "Next Tide:",
      tideNameNext,
...
function SunkenCity(isAggro) {
  ...
  if (
    ...
    if (depth >= 500) {
      var nextZoneName = getPageVariable(
        "user.quests.QuestSunkenCity.zones[2].name"
      );
      var nextZoneLeft = parseInt(
...
function labyrinth() {
  ...
  if (objLaby.disarmCompass && charmArmed.indexOf("Compass Magnet") > -1) {
    var userVariable = JSON.parse(
      getPageVariable("JSON.stringify(user.quests.QuestLabyrinth)")
    );
  ...
  if (isAtHallway) {
    var strCurHallwayFullname = document
    ...
    if (index < 0) {
      if (objLaby.chooseOtherDoors) {
        ...
        if (objLaby.typeOtherDoors.indexOf("SHORTEST") === 0) {
          ...
          } else arrTemp = objShortestLength.indices;
          ...
          if (
            ...
            if (nMinIndex > -1) arrTemp.push(nMinIndex);
          } else arrTemp = objFewestClue.indices;
        }
...
function zokor() {
  ...
  var objAncientCity = JSON.parse(
    getPageVariable("JSON.stringify(user.quests.QuestAncientCity)")
  );
  objAncientCity.boss = objAncientCity.boss.toUpperCase();
  var nIndex = objZokor.bossStatus.indexOf(objAncientCity.boss);
  console.plog(
    "District Tier:",
    objAncientCity.district_tier,
    "Boss Status:",
    objAncientCity.boss
  );
  if (objAncientCity.district_tier < 3) return;
...
function fieryWarpath(superCharm) {
  ...
  if (currentLocation.indexOf("Fiery Warpath") > -1) {
    ...
    var streak = parseInt(
      document.getElementsByClassName("streak_quantity")[0].textContent
    );
    //var streakMouse;
    ...
    if (document.getElementsByClassName("desert_general")[0]) {
      retreating =
        document
          .getElementsByClassName("desert_general")[0]
          .className.indexOf("inactive") > -1 &&
        document
          .getElementsByClassName("desert_supply")[0]
          .className.indexOf("inactive");
    }

    console.log(
      `Current Wave: ${wave}, with ${streak} streak(s) in , mice retreating? ${retreating}`
    );
...
// Warpath V2
function fw() {
  if (GetCurrentLocation().indexOf("Fiery Warpath") < 0) return;
  ...
  for (var prop in objFWAll) {
    if (objFWAll.hasOwnProperty(prop)) {
      if (assignMissingDefault(objFWAll[prop], objDefaultFW)) temp = true;
    }
  }
  ...
  if (wave == 4) {
    ...
    if (Number.isNaN(nWardenLeft)) nWardenLeft = 12;
  ...
  objFW.streakMouse = getPageVariable(
    "user.viewing_atts.desert_warpath.streak_type"
  );
  ...
  if (objFW.special[objFW.streak] == "COMMANDER") {
    ...
    if (
      objFW.streak === 0 ||
      bCurrentStreakZeroPopulation ||
      bWrongSoldierTypeStreak
    ) {
      objFW.streak = 0;
      ...
      if (
        ...
        if (objFW.charmType[objFW.streak].indexOf("Super") > -1)
          ...
          charmName = `${objFW.charmType[objFW.streak]} ${
            objPopulation.name[index]
          }`;
      }
  ...
  if (
    objFW.disarmAfterSupportRetreat &&
    sumData(objFW.population.all) <= g_arrFWSupportRetreat[wave]
  ) {
    if (charmArmed.indexOf("Warpath") > -1) disarmTrap("trinket");
  } else checkThenArm("best", "trinket", charmName);
}

function fRift() {
  if (GetCurrentLocation().indexOf("Furoma Rift") < 0) return;
  ...
  var objUserFRift = JSON.parse(
    getPageVariable("JSON.stringify(user.quests.QuestRiftFuroma)")
  );
  ...
  if (bInPagoda) {
    ...
    var nRemainingEnergy = parseInt(
      getPageVariable(
        "user.quests.QuestRiftFuroma.droid.remaining_energy"
      ).replace(/,/g, "")
    );
    ...
    console.plog(
      "In Pagoda, Current Battery Level:",
      nCurBatteryLevel,
      "Remaining Energy:",
      nRemainingEnergy
    );
    ...
    console.plog(
      "In Training Ground, Fully Charged Battery Level:",
      nFullBatteryLevel,
      "Stored Enerchi:",
      nStoredEnerchi
    );
...
function fRiftArmTrap(obj, nIndex, bReadJournal) {
  if (isNullOrUndefined(bReadJournal)) bReadJournal = true;
    ...
    if (g_arrHeirloom.length === 0) {
      ...
      var intervalFRAT = setInterval(function () {
        if (
          ...
          var classPinnacle = document.getElementsByClassName(
            "riftFuromaHUD-craftingPopup-tabContent pinnacle"
          );
          var i, temp;
          for (i = 0; i < 3; i++) {
      ...
      if (bReadJournal === true) getJournalDetailFRift();
  ...
  } else checkThenArm(null, "bait", obj.bait[nIndex]);
}

function retrieveMouseList() {
  fireEvent(document.getElementById("effectiveness"), "click");
  var sec = secWait;
  var intervalRML = setInterval(function () {
...
function gwh() {
  ...
  var bCanFly = false;
  var nAAQuantity = parseInt(
    document.getElementsByClassName(
      "winterHunt2016HUD-featuredItem-quantity"
    )[0].textContent
  );
  var nFireworkQuantity = parseInt(
...
function checkCharge2016(stopDischargeAt) {
  try {
    ...
    var charmContainer = document.getElementsByClassName(
...
function getTrapListFromTrapSelector(sort, category, name, isForcedRetry) {
  ...
  var intervalGTLFTS = setInterval(function () {
    if (isNewUI)
...
function clickThenArmTrapInterval(sort, trap, name) {
  ...
  var intervalCTATI = setInterval(function () {
    ...
    if (armStatus != LOADING) {
      ...
      --sec;
      if (sec <= 0) {
        if (isNewUI) closeTrapSelector(trap);
...
function armTrap(sort, trap, name) {
  return isNewUI
    ...
    : armTrapClassicUI(sort, trap, name);
}
    ...
    for (var i = 0; i < tagGroupElement.length; ++i) {
      ...
      for (var j = 0; j < tagElement.length; ++j) {
        ...
        if (nameElement.indexOf(name) === 0) {
          ...
          else closeTrapSelector(trap);

          if (objTrapList[trap].indexOf(nameElement) < 0) {
            objTrapList[trap].unshift(nameElement);
            setStorage(
              `TrapList${capitalizeFirstLetter(trap)}`,
              objTrapList[trap].join(",")
            );
          }
...
function armTrapNewUI(sort, trap, name) {
  var itemEle = document.getElementsByClassName(
    "campPage-trap-itemBrowser-item"
  );
  var nameElement;
  var arrName = Array.isArray(name) ? name.slice() : [name];
...
function clickTrapSelector(strSelect, bForceClick) {
  //strSelect = weapon/base/charm/trinket/bait
  if (isNullOrUndefined(bForceClick)) bForceClick = false;
  if (isNewUI) {
    var armedItem = document.getElementsByClassName(
    ...
    if (
      bForceClick !== true &&
      document.getElementsByClassName(`showComponents ${strSelect}`).length > 0
    ) {
      // trap selector opened
      arming = true;
      return console.plog("Trap selector", strSelect, "opened");
  ...
  try {
    ...
    if (scriptElementList) {
      ...
      for (i = 0; i < scriptElementList.length; ++i) {
        ...
        // get next horn time
        var hornTimeStartIndex = scriptString.indexOf(
        ...
        if (hornTimeStartIndex >= 0) {
          ...
          var hornTimerString = scriptString.substring(
            hornTimeStartIndex,
            hornTimeEndIndex
          );
          ...
          if (debug)
            console.log(
              `From substr: ${nextActiveTime}, from page var: ${getPageVariable(
                "user.next_activeturn_seconds"
              )}`
            );
        ...
        if (hasPuzzleStartIndex >= 0) {
          ...
          var hasPuzzleEndIndex = scriptString.indexOf(
            ",",
            hasPuzzleStartIndex
          );
          var hasPuzzleString = scriptString.substring(
            hasPuzzleStartIndex,
            hasPuzzleEndIndex
          );
        ...
        if (baitQuantityStartIndex >= 0) {
          ...
          var baitQuantityEndIndex = scriptString.indexOf(
            ",",
            baitQuantityStartIndex
          );
          var baitQuantityString = scriptString.substring(
            baitQuantityStartIndex,
        ...
        if (locationStartIndex >= 0) {
          ...
          var locationString = scriptString.substring(
            locationStartIndex,
            locationEndIndex
          );
...
function getBaitQuantity() {
  var hudBaitQuantity = document
    .getElementsByClassName("mousehuntHud-userStat bait")[0]
  ...
  try {
    if (isKingReward) {
      ...
      if (fbPlatform) {
        if (secureConnection) {
          displayLocation(
            `<span style="color: red; ">${currentLocation}</span> [<a onclick='window.localStorage.removeItem("huntLocation");' href="https://www.mousehuntgame.com/canvas/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
          );
        } else {
          displayLocation(
            `<span style="color: red; ">${currentLocation}</span> [<a onclick='window.localStorage.removeItem("huntLocation");' href="http://www.mousehuntgame.com/canvas/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
          );
        }
      } else if (hiFivePlatform) {
        if (secureConnection) {
          displayLocation(
            `<span style="color: red; ">${currentLocation}</span> [<a onclick='window.localStorage.removeItem("huntLocation");' href="https://mousehunt.hi5.hitgrab.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
          );
        } else {
          displayLocation(
            `<span style="color: red; ">${currentLocation}</span> [<a onclick='window.localStorage.removeItem("huntLocation");' href="http://mousehunt.hi5.hitgrab.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
          );
        }
      } else if (mhPlatform) {
        if (secureConnection) {
          displayLocation(
            `<span style="color: red; ">${currentLocation}</span> [<a onclick='window.localStorage.removeItem("huntLocation");' href="https://www.mousehuntgame.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
          );
        } else {
          displayLocation(
            `<span style="color: red; ">${currentLocation}</span> [<a onclick='window.localStorage.removeItem("huntLocation");' href="http://www.mousehuntgame.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
          );
        }
      }
...
function countdownTimer() {
  if (isKingReward) {
    ...
    if (fbPlatform) {
      if (secureConnection) {
        displayLocation(
          `<font color="red">${currentLocation}</font> [<a onclick='window.localStorage.removeItem("huntLocation");' href="https://www.mousehuntgame.com/canvas/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
        );
      } else {
        displayLocation(
          `<font color="red">${currentLocation}</font> [<a onclick='window.localStorage.removeItem("huntLocation");' href="http://www.mousehuntgame.com/canvas/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
        );
      }
    } else if (hiFivePlatform) {
      if (secureConnection) {
        displayLocation(
          `<font color="red">${currentLocation}</font> [<a onclick='window.localStorage.removeItem("huntLocation");' href="https://mousehunt.hi5.hitgrab.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
        );
      } else {
        displayLocation(
          `<font color="red">${currentLocation}</font> [<a onclick='window.localStorage.removeItem("huntLocation");' href="http://mousehunt.hi5.hitgrab.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
        );
      }
    } else if (mhPlatform) {
      if (secureConnection) {
        displayLocation(
          `<font color="red">${currentLocation}</font> [<a onclick='window.localStorage.removeItem("huntLocation");' href="https://www.mousehuntgame.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
        );
      } else {
        displayLocation(
          `<font color="red">${currentLocation}</font> [<a onclick='window.localStorage.removeItem("huntLocation");' href="http://www.mousehuntgame.com/">Hunt Here</a>] - <i>Script pause because you had move to a different location recently, click hunt here to continue hunt at this location.</i>`
        );
      }
    }
    displayKingRewardSumTime(null);
    ...
    if (hornTime <= 0) {
      ...
      if (enableTrapCheck) {
        // update timer
        displayTimer(
          `Horn: ${timeFormat(hornTime)} | Check: ${timeFormat(checkTime)}`,
          `${timeFormat(hornTime)}  <i>(included extra ${timeFormat(
            hornTimeDelay
          )} delay & +/- 5 seconds different from MouseHunt timer)</i>`,
          `${timeFormat(checkTime)}  <i>(included extra ${timeFormat(
            checkTimeDelay
          )} delay)</i>`
        );
  ...
  try {
    if (showTimerInPage) {
      ...
      if (headerElement) {
        ...
        titleElement.innerHTML = `<b>MouseHunt AutoBot (version ${scriptVersion})${
          isNewUI ? " ~ Beta UI" : ""
        }</b>`;
        ...
        if (targetPage) {
          ...
          if (lastKingRewardDate == undefined || lastKingRewardDate == null) {
            ...
            var lastDate = new Date(lastKingRewardDate);
            lastDateStr = `${lastDate.toDateString()} ${lastDate
              .toTimeString()
              .substring(0, 8)}`;
            lastDate = null;
          }
...
<tr>
  <td colspan="2" style="padding: 2px 0; border-bottom: 1px solid orange;"></td>
</tr>
<tr>
...
<tr>
  ...
  <td style="height:24px">
    <input type="text" id="HornTimeDelayMinInput" name="HornTimeDelayMinInput" value="${hornTimeDelayMin.toString()}"/> seconds
    ~
    <input type="text" id="HornTimeDelayMaxInput" name="HornTimeDelayMaxInput" value="${hornTimeDelayMax.toString()}"/> seconds
  </td>
</tr>
<tr>
  <td style="height:24px; text-align:right;">
    <a title="Enable trap check once an hour">
      <b>Trap Check</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="TrapCheckInput" onchange="var isDisable = (value == 'false') ? 'disabled' : ''; document.getElementById('TrapCheckTimeDelayMinInput').disabled=isDisable; document.getElementById('TrapCheckTimeDelayMaxInput').disabled=isDisable;">
      <option value="false" ${enableTrapCheck ? "" : "selected"}>False</option>
      <option value="true" ${enableTrapCheck ? "selected" : ""}>True</option>
    </select>
    &nbsp;&nbsp;
    <a title="Extra delay time to trap check (in seconds)">
      <b>Delay:</b>
    </a>
    &emsp;
    <input type="number" id="TrapCheckTimeDelayMinInput" min="0" max="360" size="5" value="${checkTimeDelayMin.toString()}" ${
          enableTrapCheck ? "" : "disabled"
        }> seconds
    ~
    <input type="number" id="TrapCheckTimeDelayMaxInput" min="1" max="361" size="5" value="${checkTimeDelayMax.toString()}" ${
          enableTrapCheck ? "" : "disabled"
        }> seconds
  </td>
</tr>
<tr>
  <td style="height:24px; text-align:right;">
    <a title="Play sound when encounter king's reward"><b>Play King Reward Sound</b></a>&nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="PlayKingRewardSoundInput">
      <option value="false"${
        !isKingWarningSound ? " selected" : ""
      }>False</option>
      <option value="true"${isKingWarningSound ? " selected" : ""}>True</option>
    </select>
    &nbsp;&nbsp;
    <a title="Link to MP3 sound to play, defaults NobodyRandom's awesome song if left blank">
      <b>Sound Link:</b>
    </a>
    &emsp;
...
<tr>
  ...
  <td style="height: 24px">
    <select id="autoPopKR">
      <option value="false"${!autoPopupKR ? " selected" : ""}>False</option>
      <option value="true"${autoPopupKR ? " selected" : ""}>True</option>
    </select>
  </td>
</tr>
<tr>
  <td style="height:24px; text-align:right;">
    <a title="Solve King Reward automatically">
      <b>Auto Solve King Reward</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="AutoSolveKRInput" onchange="var isDisable = (value == 'false') ? 'disabled' : ''; document.getElementById('AutoSolveKRDelayMinInput').disabled=isDisable; document.getElementById('AutoSolveKRDelayMaxInput').disabled=isDisable;">
      <option value="false" ${isAutoSolve ? "" : "selected"}>False</option>
      <option value="true" ${isAutoSolve ? "selected" : ""}>True</option>
    </select>
    &nbsp;&nbsp;
    <a title="Extra delay time to solve King Reward (in seconds)">
      <b>Delay:</b>
    </a>
    &emsp;
    <input type="number" id="AutoSolveKRDelayMinInput" min="0" max="360" size="5" value="${krDelayMin.toString()}" ${
          isAutoSolve ? "" : "disabled"
        }> seconds
    ~
    <input type="number" id="AutoSolveKRDelayMaxInput" min="1" max="361" size="5" value="${krDelayMax.toString()}" ${
          isAutoSolve ? "" : "disabled"
        }> seconds
  </td>
</tr>
<tr>
  <td style="height:24px; text-align:right;">
    <a title="The script will pause if player at different location that hunt location set before">
      <b>Remember Location</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="PauseLocationInput" >
      <option value="false" ${
        pauseAtInvalidLocation ? "" : "selected"
      }>False</option>
      <option value="true" ${
        pauseAtInvalidLocation ? "selected" : ""
      }>True</option>
...
<tr>
  ...
  <td style="height:24px">
    <select id="selectBestTrapWeapon" style="width: 300px" onchange="saveBestTrap();"></select>
  </td>
</tr>
<tr>
  <td style="height:24px; text-align:right;">
  ...
  <td style="height:24px">
    <select id="selectBestTrapBase" onchange="saveBestTrap();"></select>
  </td>
</tr>
<tr>
  <td style="height:24px; text-align:right;" colspan="2">
    ...
    <input type="button" id="PreferenceSaveInput" value="Save" onclick="
      ...
      if (${fbPlatform})
        window.location.href='${g_strHTTP}://www.mousehuntgame.com/canvas/';
      else if (${hiFivePlatform})
        window.location.href='${g_strHTTP}://www.mousehunt.hi5.hitgrab.com/';
      else if (${mhPlatform})
        window.location.href='${g_strHTTP}://www.mousehuntgame.com/';">
    &nbsp;&nbsp;&nbsp;
  </td>
</tr>
...
<tr>
  <td style="height:24px; text-align:right;">
    <a title="Turn on/off Map Hunting feature">
      <b>Season 4 Map Hunting</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trUncaughtMouse" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Click button Get to retrieve all uncaught mouse">
      <b>Uncaught Mouse</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trSelectedUncaughtMouse" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select desired uncaught mouse">
      <b>Selected Mouse</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trCatchLogic" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select desired catch logic">
      <b>Catch Logic</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trMapHuntingTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup after catch logic is fulfilled">
      <b>After Caught</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trMapHuntingLeave" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to leave map after catch logic is fulfilled">
      <b>Leave Map After Caught</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr>
  <td style="height:24px; text-align:right;">
    <a title="Select the script algorithm based on certain event / location">
      <b>Event or Location</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trBCJODTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup based on current sub-location">
      <b>Trap Setup </b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="selectBCJODWeapon" style="width: 75px;" onchange="saveBCJOD();"></select>
    <select id="selectBCJODBase" style="width: 75px;" onchange="saveBCJOD();"></select>
...
<tr id="trFGARTrapSetup" style="display:none;">
  ...
  <td style="height:24px">
    <select id="selectFGARWeapon" style="width: 75px;" onchange="saveFGAR();"></select>
    <select id="selectFGARBase" style="width: 75px;" onchange="saveFGAR();"></select>
...
<tr id="trBWRiftAutoChoosePortal" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Choose portal automatically">
      <b>Auto Choose Portal</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trBWRiftChoosePortalAfterCC" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Choose portal after Chamber Cleaver has been caught">
      <b>Choose Portal</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    &nbsp;&nbsp;After Chamber Cleaver Caught
  </td>
</tr>
<tr id="trBWRiftPortalPriority" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select portal priority">
      <b>Portal Priority </b>
    </a>
...
<tr id="trBWRiftPortalPriorityCursed" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select portal priority when get cursed">
      <b>Portal Priority - Cursed </b>
    </a>
...
<tr id="trBWRiftMinTimeSand" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select minimum time sand before entering Acolyte Chamber (AC)">
      <b>Min Time Sand </b>
    </a>
  ...
  <td style="height:24px">
    <input type="number" id="inputMinTimeSand" min="0" max="99999" style="width:75px" value="50" onchange="onInputMinTimeSandChanged(this);">
    &nbsp;&nbsp;Before Enter AC
  </td>
</tr>
<tr id="trBWRiftMinRSC" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select minimum Runic String Cheese before entering Acolyte Chamber (AC)&#13;Note 1: Total RSC = 2*RSC Pot + RSC">
      <b>Min Runic String Cheese</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    <input type="number" id="inputMinRSC" min="0" max="99999" style="width:75px" value="50" onchange="onInputMinRSCChanged(this);">
    &nbsp;&nbsp;Before Enter AC
  </td>
</tr>
<tr id="trBWRiftEnterMinigame" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to enter minigame with curse(s)">
      <b>Enter Minigame with Curse(s)</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trBWRiftSubLocation" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Chamber in Bristle Woods Rift">
      <b>Sub-Location</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trBWRiftMasterTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup based on current chamber">
      <b>Master Trap Setup</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="selectBWRiftWeapon" style="width: 75px;" onchange="saveBWRift();">
      ...
      <option value="Celestial Dissonance Trap">CDT</option>
      ...
      <option value="Christmas Crystalabra Trap">Christmas Crystalabra Trap</option>
    ...
    <select id="selectBWRiftBait" style="width: 75px;" onchange="saveBWRift();">
      <option value="None">None</option>
      <option value="Runic String">Runic</option>
      <option value="Ancient String">Ancient</option>
      <option value="Runic/Ancient">Runic/Ancient</option>
      <option value="Runic=>Ancient">Runic=>Ancient</option>
...
<tr id="trBWRiftTrapSetupSpecial" style="display:none;">
  <td style="height:24px; text-align:right;"><a title="Select trap setup based on current chamber"><b>Conditional Trap Setup </b></a>
...
<tr id="trBWRiftActivatePocketWatch" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Activate Quantum Pocketwatch forcibly">
      <b>Force Activate Quantum</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    &nbsp;&nbsp;If Remaining Loot/Obelisk Charge &le;&nbsp;&nbsp;:&nbsp;&nbsp;
...
<tr id="trBWRiftDeactivatePocketWatch" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Deactivate Quantum Pocketwatch forcibly">
      <b>Force Deactivate Quantum</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trFRoxTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;"><a title="Select trap setup based on current stage"><b>Trap Setup for </b></a>
  ...
  <td style="height:24px">
    <select id="selectFRoxWeapon" style="width: 75px;" onchange="saveFRox();"></select>
    <select id="selectFRoxBase" style="width: 75px;" onchange="saveFRox();"></select>
...
<tr id="trFRoxDeactiveTower" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to deactivate tower when full HP">
      <b>Deactivate Tower When HP Full</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trGESTrapSetup" style="display:none;">
  ...
  <td style="height:24px">
    <select id="selectGESTrapWeapon" style="width: 75px;" onchange="saveGES();">
      ...
      <option value="Christmas Cactus Trap">Christmas Cactus Trap</option>
      <option value="Meteor Prison Core Trap">Meteor Prison Core Trap</option>
      <option value="S.T.I.N.G. Trap">S.T.I.N.G. Trap</option>
      <option value="S.T.I.N.G.E.R. Trap">S.T.I.N.G.E.R. Trap</option>
    </select>
    <select id="selectGESTrapBase" style="width: 75px" onchange="saveGES();"></select>
    ...
    <select id="selectGESDCTrapTrinket" style="width: 75px;display:none" onchange="saveGES();">
      <option value="None">None</option>
      <option value="AUTO">Magmatic Crystal/Black Powder/Dusty Coal</option>
    </select>
...
<tr id="trWWRiftFactionFocus" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select a faction to focus on">
      <b>Faction to Focus</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trWWRiftFactionFocusNext" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select next faction to focus on">
      <b>Next Faction to Focus</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trWWRiftTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to trap setup based on certain range of rage">
      <b>Trap Setup for Rage</b>
    </a>
  ...
  <td style="height:24px">
    ...
    <select id="selectWWRiftTrapBase" style="width: 75px" onchange="saveWWRift();"></select>
...
<tr id="trWWRiftMBWMinRage" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select minimum rage to hunt MBW">
      <b>Min Rage to Hunt MBW</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trWWRiftMBWTrapSetup" style="display:none;">
  ...
  <td style="height:24px">
    ...
    <select id="selectWWRiftMBWTrapBase" style="width: 75px" onchange="saveWWRift();"></select>
...
<tr id="trFREnterBattery" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select which battery level to enter Pagoda">
      <b>Enter at Battery</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trFRRetreatBattery" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select which battery level to retreat from  Pagoda">
      <b>Retreat at Battery</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trIceberg" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to trap setup based on current phase">
      <b>Trap Setup for</b>
    </a>
  ...
  <td style="height:24px">
    <select id="selectIcebergBase" style="width: 75px" onchange="saveIceberg();"></select>
...
<tr id="trZTFocus" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to chesspiece side to focus">
      <b>Side to Focus</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trZTTrapSetup1st" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup based on first focus-side chesspiece order">
      <b>First Side Trap Setup for </b>
    </a>
  ...
  <td style="height:24px">
    ...
    <select id="selectZTBase1st" style="width: 75px" onchange="saveZT();"></select>
...
<tr id="trZTTrapSetup2nd" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup based on second focus-side chesspiece order">
      <b>Second Side Trap Setup for </b>
    </a>
  ...
  <td style="height:24px">
    ...
    <select id="selectZTBase2nd" style="width: 75px" onchange="saveZT();"></select>
...
<tr id="trSGTrapSetup" style="display:none;">
  ...
  <td style="height:24px">
    <select id="selectSGTrapWeapon" style="width: 75px" onchange="saveSG();"></select>
    <select id="selectSGTrapBase" style="width: 75px" onchange="saveSG();"></select>
...
<tr id="trSGDisarmBait" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to disarm bait when amplifier is fully charged">
      <b>Disarm Bait</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trPourTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;"><a><b>After Poured in </b></a>
  ...
  <td style="height:24px">
    <select id="selectLGTGBase" style="width: 75px" onchange="saveLG();"></select>
...
<tr id="trCurseLiftedTrapSetup" style="display:none;">
  ...
  <td style="height:24px">
    <select id="selectLCCCBase" style="width: 75px" onchange="saveLG();"></select>
...
<tr id="trSaltedTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;"><a><b>Trap Setup </b></a>
  ...
  <td style="height:24px">
    <select id="selectSCBase" style="width: 75px" onchange="saveLG();"></select>
    &nbsp;&nbsp;
    <a title="Max number of salt before hunting King Grub">
      <b>Salt Charge : </b>
    </a>
...
<tr id="trGWHTrapSetup" style="display:none;">
  ...
  <td style="height:24px">
    <select id="selectGWHWeapon" style="width: 75px" onchange="saveGWH2016();"></select>
    <select id="selectGWHBase" style="width: 75px" onchange="saveGWH2016();"></select>
    <select id="selectGWHTrinket" style="width: 75px;" onchange="onSelectGWHTrinketChanged();">
      <option value="None">None</option>
      <option value="ANCHOR_FAC/EAC">FAC/EAC</option>
    </select>
    ...
    <select id="selectGWHBoost" style="width: 75px" onchange="saveGWH2016();">
      <option value="false">Not Boost</option>
      <option value="true">Boost</option>
    </select>
  </td>
</tr>

<tr id="trGWHTurboBoost" style="display:none;">
...
<tr id="trGWHFlying" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select minimum AA to take flight">
      <b>Min AA to Fly (&ge;)</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trGWHFlyingFirework" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select minimum firework to take flight">
      <b>Min Firework to Fly (&ge;)</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trGWHFlyingLand" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select whether land after firework run out">
      <b>Land after Firework Run Out</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
    </td>
  <td style="height:24px">
...
<tr id="trSCCustom" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select custom algorithm">
      <b>SC Custom Algorithm</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trSCCustomUseSmartJet" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to always use Smart Water Jet Charm">
      <b>Use Smart Jet</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trLabyrinth" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select a district to focus on">
      <b>District to Focus</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trLabyrinthDisarm" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to disarm cheese at X last hunt in hallway when total clues near 100">
      <b>Security Disarm</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trLabyrinthArmOtherBase" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select to arm other base if Compass Magnet Charm is currently armed">
      <b>Arm Other Base</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    &nbsp;&nbsp;If Compass Magnet Charm is armed
  </td>
</tr>
...
<tr id="trPriorities15" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select hallway priorities when focus-district clues less than 15">
      <b>Priorities (Focus-District Clues < 15)</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trPriorities1560" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select hallway priorities when focus-district clues within 15 and 60">
      <b>Priorities (15 < Focus-District Clues < 60)</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trPriorities60" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select hallway priorities when focus-district clues more than 60">
      <b>Priorities (Focus-District Clues > 60)</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
...
<tr id="trLabyrinthOtherHallway" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Choose doors other than focused door when there is no available focused door to be choosen">
      <b>Open Non-Focus Door</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    &nbsp;&nbsp;
    <a title="Select a choosing type for non-focused doors">
      <b>Choosing Type:</b>
    </a>
    &emsp;
...
<tr id="trLabyrinthWeaponFarming" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select weapon type in farming hallways">
      <b>Weapon Type in Farming</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="selectLabyrinthWeaponType" onchange="saveLaby();">
      <option value="Forgotten">Forgotten</option>
      <option value="Arcane">Arcane</option>
    </select>
  </td>
...
<tr id="trZokorTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup under different boss status">
      <b>Trap Setup When</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trFWTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup based on certain FW wave">
      <b>Physical Trap Setup</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    <select id="selectFWTrapSetupWeapon" style="width: 75px" onchange="saveFW();"></select>
    <select id="selectFWTrapSetupBase" style="width: 75px" onchange="saveFW();"></select>
  </td>
</tr>

<tr id="trFW4TrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup based on warden status">
      <b>Trap Setup </b>
    </a>
  ...
  <td style="height:24px">
    <select id="selectFW4TrapSetupWeapon" style="width: 75px" onchange="saveFW();"></select>
    <select id="selectFW4TrapSetupBase" style="width: 75px" onchange="saveFW();"></select>
...
<tr id="trFWFocusType" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select either Normal (Warrior, Scout, Archer) or Special (Cavalry, Mage)">
      <b>Soldier Type to Focus</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    &nbsp;&nbsp;
    <a title="Select which soldier type comes first based on population">
      <b>Priorities:</b>
    </a>
    &emsp;
...
<tr id="trFWLastType" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select config when there is only one soldier type left">
      <b>Last Soldier Type</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px;">
    ...
    &nbsp;&nbsp;
    <a title="Select whether to include Artillery in checking of Last Soldier">
      <b>Include Artillery:</b>
    </a>
    &emsp;
...
<tr id="trFWSupportConfig" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select whether to disarm any Warpath Charm when supports are gone">
      <b>Disarm Warpath Charm</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px">
    ...
    &nbsp;&nbsp;When Support Retreated
  </td>
</tr>

<tr id="trBRConfig" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select the mist tier to hunt">
      <b>Hunt At</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
...
<tr id="trBRToggle" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select the amount of hunt to toggle canister">
      <b>Toggle Canister Every</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px;">
...
<tr id="trBRTrapSetup" style="display:none;">
  <td style="height:24px; text-align:right;">
    <a title="Select trap setup combination for respective mist tier">
      <b>Trap Setup</b>
    </a>
    &nbsp;&nbsp;:&nbsp;&nbsp;
  </td>
  <td style="height:24px;">
    ...
    <select id="selectBRTrapBase" style="width: 75px" onchange="saveBR();"></select>
...
${
  ...
  <tr>
    <td style="height:24px; text-align:right;">
      <a title="Type in how many hunts you want to hunt for">
        <b>How many hunts?</b>
      </a>
      &nbsp;&nbsp;:&nbsp;&nbsp;
    </td>
    <td style="height:24px">
      <input type="number" id="nobHuntsLeftInput" name="nobHuntsLeftInput" value="${NOBhuntsLeft}" />
    </td>
  </tr>
`
    : ""
...
function afterSoundingHorn() {
  ...
  if (headerElement) {
    ...
    if (headerStatus.indexOf(hornReady) != -1) {
      ...
      // update timer
      displayTimer(
        "The horn sounding taken extra longer than normal...",
        "The horn sounding taken extra longer than normal...",
        "The horn sounding taken extra longer than normal..."
      );
...
function embedScript() {
  ...
  var modStr = `${oriStr.substring(0, index)}soundedHorn();${oriStr.substring(
    index
  )}`;
...
function kingRewardCountdownTimer(interval, isReloadToSolve) {
  ...
  if (interval < 0) {
    ...
    if (isReloadToSolve) {
      if (debug)
        console.log("Reloading to solve KR, clicking on campElement now");
      ...
      var intervalCRB = setInterval(function () {
...
function CalculateNextTrapCheckInMinute() {
  if (enableTrapCheck) {
    ...
    checkTimeDelay =
      checkTimeDelayMin +
      Math.round(Math.random() * (checkTimeDelayMax - checkTimeDelayMin));
...
function disarmTrap(trapSelector) {
  ...
  var x;
  var strTemp = "";
  var intervalDisarm = setInterval(function () {
    if (arming === false) {
      addArmingIntoList(trapSelector);
      clickTrapSelector(trapSelector);
      var intervalDT = setInterval(function () {
        if (isNewUI) {
    ...
    try {
      ...
      if (NOBdata != null || NOBdata != undefined) {
        if (!mapRequestFailed && mapThere) {
          ...
          console.log(
            `Map fetch failed, using USER data from html (${mapRequestFailed}, ${mapThere})`
          );
          nobHTMLFetch();
    ...
    Parse.User.logIn(theUsername, thePassword)
      .then(
        ...
        function (user, error) {
          if (debug)
            console.log(
              "Parse login failed, attempting to create new user now."
            );
          ...
          createUser.signUp(null, {
            success: function (newUser) {
              ...
              return Parse.Promise.error(
                "Creating new user, trying to login now."
              );
            },
            error: function (newUser, signupError) {
              ...
              if (debug)
                console.log(
                  `Parse Error: ${signupError.code} ${signupError.message}`
                );
              return Parse.Promise.error(
                "Error in signup, giving up serverPing now."
              );
            },
          });
          return Parse.Promise.error(
            "Failed login, attempted signup, rerunning code"
          );
        }
      )
...
function nobRaffle() {
  ...
  var nobRafInt = window.setInterval(function () {
    try {
      ...
      console.log(
        `Raffle interval error: ${e}, retrying in 2 seconds. Giving up in ${
          nobRafGiveUp * 2
        } seconds.`
      );
      if (nobRafGiveUp < 1) {
...
function nobPresent() {
  ...
  var nobPresInt = window.setInterval(function () {
    try {
      ...
      console.log(
        `Present interval error: ${e}, retrying in 2 seconds. Giving up in ${
          nobPresGiveUp * 2
        } seconds.`
      );
      if (nobPresGiveUp < 1) {
...
function bodyJS() {
  ...
  function onIdAdsClicked() {
    ...
    xmlHttp.open(
      "GET",
      "https://dl.dropboxusercontent.com/s/3cbo6en86lrpas1/Test.txt",
      true
  ...
  function initControlsMapHunting() {
    ...
    var trSelectedUncaughtMouse = document.getElementById(
      "trSelectedUncaughtMouse"
    );
    var trCatchLogic = document.getElementById("trCatchLogic");
    var selectMapHunting = document.getElementById("selectMapHunting");
    var selectMouseList = document.getElementById("selectMouseList");
    var trMapHuntingTrapSetup = document.getElementById(
  ...
  function initControlsBestTrap() {
    var selectBestTrapPowerType = document.getElementById(
      "selectBestTrapPowerType"
    );
    var selectBestTrapWeapon = document.getElementById("selectBestTrapWeapon");
    var selectBestTrapBaseType = document.getElementById(
  ...
  function initControlsGWH2016(bAutoChangeZone) {
    ...
    var selectGWHUseTurboBoost = document.getElementById(
    ...
    var selectGWHLandAfterRunOutFirework = document.getElementById(
  ...
  function initControlsSCCustom(bAutoChangeZone) {
    ...
    var selectSCHuntZoneEnable = document.getElementById(
  ...
  function onSelectLabyrinthDisarm() {
    var inputLabyrinthLastHunt = document.getElementById(
      "inputLabyrinthLastHunt"
    );
    var selectLabyrinthDisarm = document.getElementById(
  ...
  function saveLG() {
    var selectLGTGAutoFillSide = document.getElementById(
      "selectLGTGAutoFillSide"
    );
    var selectLGTGAutoFillState = document.getElementById(
      "selectLGTGAutoFillState"
    );
    var selectLGTGAutoPourSide = document.getElementById(
      "selectLGTGAutoPourSide"
    );
    var selectLGTGAutoPourState = document.getElementById(
  ...
  function initControlsLG(bAutoChangeLocation) {
    ...
    if (isNullOrUndefined(storageValue)) {
      ...
      selectLGTGTrinket.value =
        storageValue[selectLGTGSide.value].trinket.after;
      ...
      selectLCCCTrinket.value =
        storageValue[selectLCCCSide.value].trinket.after;
  ...
  function initControlsFW(bAutoChangeWave) {
    ...
    var selectFWTrapSetupWeapon = document.getElementById(
      "selectFWTrapSetupWeapon"
    );
    var selectFWTrapSetupBase = document.getElementById(
      "selectFWTrapSetupBase"
    );
    ...
    var selectFWLastTypeConfig = document.getElementById(
      "selectFWLastTypeConfig"
    );
    var selectFWLastTypeConfigIncludeArtillery = document.getElementById(
      "selectFWLastTypeConfigIncludeArtillery"
    );
    var selectFWSupportConfig = document.getElementById(
      "selectFWSupportConfig"
    );
    var selectFW4WardenStatus = document.getElementById(
      "selectFW4WardenStatus"
    );
    var selectFW4TrapSetupWeapon = document.getElementById(
      "selectFW4TrapSetupWeapon"
    );
    var selectFW4TrapSetupBase = document.getElementById(
      "selectFW4TrapSetupBase"
    );
    var selectFW4TrapSetupTrinket = document.getElementById(
      "selectFW4TrapSetupTrinket"
    );
    var selectFW4TrapSetupBait = document.getElementById(
  ...
  function saveZokor() {
    var selectZokorBossStatus = document.getElementById(
  ...
  function saveFR() {
    ...
    var selectRetreatAtBattery = document.getElementById(
      "selectRetreatAtBattery"
    );
    var nIndex = document.getElementById(
    ...
    var selectFRTrapBaitMasterOrder = document.getElementById(
  ...
  function initControlsFR(bAutoChangeBatteryLevel) {
    ...
    var selectTrapSetupAtBattery = document.getElementById(
      "selectTrapSetupAtBattery"
    );
  ...
  function saveFGAR() {
    var selectFGARSublocation = document.getElementById(
  ...
  function saveBCJOD() {
    var selectBCJODSublocation = document.getElementById(
  ...
  function saveBWRift() {
    ...
    var selectBWRiftActivatePocketWatch = document.getElementById(
      "selectBWRiftActivatePocketWatch"
    );
    var selectBWRiftCleaverStatus = document.getElementById(
      "selectBWRiftCleaverStatus"
    );
    var selectBWRiftAlertLvl = document.getElementById("selectBWRiftAlertLvl");
    var selectBWRiftWeaponSpecial = document.getElementById(
      "selectBWRiftWeaponSpecial"
    );
    var selectBWRiftBaseSpecial = document.getElementById(
      "selectBWRiftBaseSpecial"
    );
    var selectBWRiftBaitSpecial = document.getElementById(
      "selectBWRiftBaitSpecial"
    );
    var selectBWRiftTrinketSpecial = document.getElementById(
      "selectBWRiftTrinketSpecial"
    );
    var selectBWRiftActivatePocketWatchSpecial = document.getElementById(
      "selectBWRiftActivatePocketWatchSpecial"
    );
    var selectBWRiftForceActiveQuantum = document.getElementById(
      "selectBWRiftForceActiveQuantum"
    );
    var inputRemainingLootA = document.getElementById("inputRemainingLootA");
    var selectBWRiftForceDeactiveQuantum = document.getElementById(
      "selectBWRiftForceDeactiveQuantum"
    );
    var inputRemainingLootD = document.getElementById("inputRemainingLootD");
    var selectBWRiftChoosePortal = document.getElementById(
      "selectBWRiftChoosePortal"
    );
    var selectBWRiftChoosePortalAfterCC = document.getElementById(
      "selectBWRiftChoosePortalAfterCC"
    );
    var selectBWRiftPriority = document.getElementById("selectBWRiftPriority");
    var selectBWRiftPriorityCursed = document.getElementById(
      "selectBWRiftPriorityCursed"
    );
    var selectBWRiftPortal = document.getElementById("selectBWRiftPortal");
    var selectBWRiftBuffCurse = document.getElementById(
      "selectBWRiftBuffCurse"
    );
    var inputMinTimeSand = document.getElementById("inputMinTimeSand");
    var selectBWRiftMinRSCType = document.getElementById(
      "selectBWRiftMinRSCType"
    );
    var inputMinRSC = document.getElementById("inputMinRSC");
    var selectBWRiftEnterWCurse = document.getElementById(
  ...
  function saveFRox() {
    ...
    var selectFRoxActivateTower = document.getElementById(
      "selectFRoxActivateTower"
    );
    var selectFRoxFullHPDeactivate = document.getElementById(
  ...
  function saveWWRift() {
    var selectWWRiftFaction = document.getElementById("selectWWRiftFaction");
    var selectWWRiftFactionNext = document.getElementById(
      "selectWWRiftFactionNext"
    );
    var selectWWRiftRage = document.getElementById("selectWWRiftRage");
    var selectWWRiftTrapWeapon = document.getElementById(
      "selectWWRiftTrapWeapon"
    );
    var selectWWRiftTrapBase = document.getElementById("selectWWRiftTrapBase");
    var selectWWRiftTrapTrinket = document.getElementById(
      "selectWWRiftTrapTrinket"
    );
    var selectWWRiftTrapBait = document.getElementById("selectWWRiftTrapBait");
    var selectWWRiftMBWBar4044 = document.getElementById(
      "selectWWRiftMBWBar4044"
    );
    var selectWWRiftMBWBar4548 = document.getElementById(
      "selectWWRiftMBWBar4548"
    );
    var selectWWRiftMBWTrapWeapon = document.getElementById(
      "selectWWRiftMBWTrapWeapon"
    );
    var selectWWRiftMBWTrapBase = document.getElementById(
      "selectWWRiftMBWTrapBase"
    );
    var selectWWRiftMBWTrapTrinket = document.getElementById(
      "selectWWRiftMBWTrapTrinket"
    );
    var selectWWRiftMBWTrapBait = document.getElementById(
  ...
  function saveGES() {
    ...
    var selectGESSDTrapTrinket = document.getElementById(
      "selectGESSDTrapTrinket"
    );
    var selectGESRRTrapTrinket = document.getElementById(
      "selectGESRRTrapTrinket"
    );
    var selectGESDCTrapTrinket = document.getElementById(
      "selectGESDCTrapTrinket"
    );
    ...
    var selectGESDCStokeEngine = document.getElementById(
  ...
  function showOrHideTr(algo) {
    ...
    if (!isNullOrUndefined(objTableRow[algo])) objTableRow[algo].init(true);
